		...
		try {
			if(hospedeVO.getIdHospede() != null) {
				if(hospedeController.atualizar(hospedeVO)) {
					JOptionPane.showMessageDialog(null, "Hóspede atualizado com sucesso!", "Sucesso", JOptionPane.INFORMATION_MESSAGE);
					retorno = true;
				} else {
					JOptionPane.showMessageDialog(null, "Ocorreu um erro ao atualizar hóspede. Verifique os dados e tente novamente", "Erro", JOptionPane.ERROR_MESSAGE);
				}
			} else {
				hospedeController.inserir(hospedeVO);
				JOptionPane.showMessageDialog(null, "Hóspede criado com sucesso!", "Sucesso", JOptionPane.INFORMATION_MESSAGE);
				retorno = true;
			}
